...
1. **Create HTML/CSS banner** directly (no external skill dependency)
   - Use exact platform dimensions from size reference
   - Apply safe zone rules (critical content in central 70-80%)
   - Max 2 typefaces, single CTA, 4.5:1 contrast ratio
   - If brand guidelines exist at `docs/brand-guidelines.md`, apply them

2. **Generate visual elements** (optional — use if `/nano-banana` skill is available)

   **If Gemini API available** (check for GEMINI_API_KEY in .env):
   Use `/nano-banana` skill for AI image generation with appropriate prompts.

   **If no Gemini API**: Design using CSS-only visuals:
   - Gradient backgrounds, geometric patterns, glassmorphism effects
   - SVG illustrations and icons
   - These are often sufficient for professional banners
...
3. **Compose final banner** — overlay text, CTA, logo on generated/CSS visual in HTML/CSS

### Step 4: Export Banners to Images

After designing HTML banners, export each to PNG:

**Option A: Playwright MCP (preferred — use `browser_navigate` + `browser_take_screenshot`)**
```
browser_navigate file:///absolute/path/to/banner-01.html
browser_take_screenshot  # saves visual confirmation
```

**Option B: Manual export**
1. Open HTML file in browser
2. Use DevTools → Device Mode → set exact dimensions
3. Screenshot via DevTools (Ctrl+Shift+P → "Capture screenshot")

**Option C: Sharp CLI (for image processing)**
```bash
npx sharp-cli resize banner-source.png --width 1500 --height 500 --output banner-final.png
```
